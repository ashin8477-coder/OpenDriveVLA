...
class NuScenesE2EDataset(NuScenesDataset):
    ...
    def __init__(self,
        ...
        if os.path.exists('data/nusc.pkl'):  
            ...
            print(">>> NuScenes found, loading...")
            with open('data/nusc.pkl', 'rb') as f:
                self.nusc = pickle.load(f)
            end_time = time.time()
            print(f">>> NuScenes loaded in {end_time - start_time:.2f} seconds")
        else:
        ...
        self.eval_sample_tokens = self._build_eval_sample_tokens()
        self.token_to_info = {
            str(info['token']): info for info in self.data_infos
        }
        if self.is_debug:
            flag_length = min(len(self.data_infos), self.len_debug)
        else:
            flag_length = len(self.data_infos)
        self.flag = np.ones(flag_length, dtype=np.uint8)
        self.occ_only_total_frames = 7  # NOTE: hardcode, consistent with occupancy pipeline

    def _resolve_prediction_token(self, pred):
        """Resolve a prediction entry to a nuScenes sample token string."""

        def _coerce_token(raw):
            if raw is None:
                return None
        ...
        def _lookup_by_index(index_key, container):
            idx = container.get(index_key)
            ...
            try:
                ...
                return None
            if 0 <= idx < len(self.data_infos):
                token = self.data_infos[idx].get('token')
                if token:
                    return str(token)
            return None

        def _try_dict(container):
            if not isinstance(container, dict):
                return None

            token = container.get('token') or container.get('sample_token')
            token = _coerce_token(token)
            if token:
                return token

            token = _lookup_by_index('sample_idx', container)
            if token:
                return token

            token = _lookup_by_index('sample_id', container)
            if token:
                return token

            token = _lookup_by_index('frame_id', container)
            if token:
                return token

            return None

        token = _try_dict(pred)
        if token:
            return token

        if isinstance(pred, dict):
            nested_keys = [
                'pts_bbox', 'img_bbox', 'pts_track', 'bbox_results', 'bbox',
                'det', 'pred_dict', 'results', 'pts'
            ]
            for key in nested_keys:
                ...
                if isinstance(sub, dict):
                    # some structures nest another dict under 'data' etc.
                    for sub_key in ('data', 'meta', 'outputs'):
                        token = _try_dict(sub.get(sub_key) if isinstance(sub.get(sub_key), dict) else None)
                        if token:
                            return token

        return None

    def _unwrap_prediction_entry(self, pred):
        """Extract the innermost dict containing detection outputs."""
        if pred is None:
            return None
        ...
        while stack:
            ...
            if isinstance(current, dict):
                if 'boxes_3d' in current and current['boxes_3d'] is not None:
                    return current
                for key in (
                        'pts_bbox', 'img_bbox', 'pts_track', 'bbox_results',
                        'bbox', 'det', 'pred_dict', 'results', 'pts',
                        'output', 'outputs'):
                    if key in current:
    ...
    def _build_eval_sample_tokens(self):
        """Collect sample tokens used for evaluation (key frames in eval split)."""
        split_map = {
            'v1.0-mini': 'mini_val',
            'v1.0-trainval': 'val',
            'v1.0-test': 'test',
        ...
        split_name = split_map.get(self.version, 'val')
        split_scenes = create_splits_scenes()
        scene_names = set(split_scenes.get(split_name, []))
        ...
        for info in self.data_infos:
            ...
            sample_is_key_frame = info.get('is_key_frame')
            if scene_name is not None:
                if scene_name not in scene_names:
                    continue
                if sample_is_key_frame is not None and not bool(sample_is_key_frame):
                    continue
                eval_tokens.add(str(token))
                continue
            try:
            ...
            try:
                ...
                continue
            if scene.get('name') not in scene_names:
                continue
            if not sample.get('is_key_frame', False):
                continue
            eval_tokens.add(str(token))
    ...
    def load_annotations(self, ann_file):
        ...
        if self.file_client_args['backend'] == 'disk':
            ...
            # make sure ann_file is a string
    ...
    def union2one(self, queue):
        ...
        # timestamp_list = [to_tensor(each['timestamp']) for each in queue]  # float32 is not enough for timestamp
        # use float64 to avoid precision loss
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for det in mmcv.track_iter_progress(results):
            ...
            if isinstance(det, dict):
                token = det.get('token') or det.get('sample_token')
            if token is None and isinstance(det, dict):
                token = self._resolve_prediction_token(det)
            if token is None:
                continue
            token = str(token)
            ...
            if self.eval_sample_tokens and str(sample_token) not in self.eval_sample_tokens:
                continue
            processed_samples += 1

            det_original = det
            det = self._unwrap_prediction_entry(det)

            if det is None or not isinstance(det, dict):
                nusc_annos[sample_token] = annos
                continue

            det.setdefault('sample_token', sample_token)
            det.setdefault('token', sample_token)

            if 'map' in self.eval_mod and 'ret_iou' in det:
                map_annos = {}
            ...
            boxes, keep_idx = lidar_nusc_box_to_global(info, boxes,
                                                       mapped_class_names,
            ...
            for i, box in enumerate(boxes):
                ...
                traj_container = det_original if isinstance(det_original, dict) else {}
                if 'traj' not in traj_container and isinstance(det, dict):
                    traj_container = det
                if 'traj' in traj_container:
                    traj_local = traj_container['traj'][keep_idx[i]].numpy()[..., :2]
                    traj_scores = traj_container['traj_scores'][keep_idx[i]].numpy()
                else:
        ...
        print(f'[Debug] _format_bbox: processed {processed_samples} eval samples, '
              f'{samples_with_boxes} with boxes.')
    ...
    def format_results(self, results, jsonfile_prefix=None):
        ...
        print(f'[Debug] format_results: received {len(results)} result entries.')
        print(f'[Debug] eval_sample_tokens size: {len(self.eval_sample_tokens)}')
        results_for_format = []
        tokenised_results = []
        fallback_results = []
        for res in results:
            if not isinstance(res, dict):
                continue
            token = self._resolve_prediction_token(res)
            if token is None:
                fallback_results.append(res)
                continue
            tokenised_results.append((token, res))

        if tokenised_results:
            predicted_tokens = {token for token, _ in tokenised_results}
            if self.eval_sample_tokens:
                effective_tokens = self.eval_sample_tokens & predicted_tokens
            else:
                effective_tokens = predicted_tokens

            filtered_results = [res for token, res in tokenised_results if token in effective_tokens]

            if self.eval_sample_tokens and len(effective_tokens) != len(self.eval_sample_tokens):
                missing = self.eval_sample_tokens - effective_tokens
                if missing:
                    print(f'[Warn] {len(missing)} evaluation tokens missing from predictions.')

            if not filtered_results and fallback_results:
                # fall back to sequential alignment
                print('[Info] Unable to resolve prediction tokens; falling back to sequential alignment.')
                tokenised_results = []
            else:
                if not filtered_results:
                    print('[Warn] No predictions matched evaluation tokens; evaluation will be skipped.')
                results_for_format = filtered_results
        else:
            tokenised_results = []

        if not tokenised_results:
            if len(results) > len(self):
                results_for_format = results[-len(self):]
            else:
                results_for_format = results
            resolved_tokens = set()
            extra_tokens = set()
        else:
            resolved_tokens = {token for token, _ in tokenised_results}
            extra_tokens = resolved_tokens - (self.eval_sample_tokens or set())
            results_for_format = filtered_results
            if self.eval_sample_tokens and len(effective_tokens) != len(self.eval_sample_tokens):
                missing_tokens = list((self.eval_sample_tokens or set()) - resolved_tokens)[:10]
                unresolved_samples = len(self.eval_sample_tokens or []) - len(resolved_tokens)
                print(f'[Debug] format_results: resolved {len(resolved_tokens)} tokens, '
                      f'missing {max(unresolved_samples, 0)}. Sample missing examples: {missing_tokens}')
                print(f'[Debug] format_results: found {len(extra_tokens)} extra tokens (not in eval set).')

        missing_tokens_full = (self.eval_sample_tokens or set()) - resolved_tokens
        if missing_tokens_full:
            print(f'[Info] format_results: padding {len(missing_tokens_full)} missing tokens with empty predictions.')
            for token in missing_tokens_full:
                results_for_format.append({'token': token, 'boxes_3d': None})
        if not results_for_format:
            print('[Warn] Prediction results list is empty after filtering/trimming.')
        else:
            print(f'[Debug] format_results: using {len(results_for_format)} entries after filtering.')
        ...
        result_files = self._format_bbox(results_for_format, jsonfile_prefix)
    ...
    def _format_bbox_det(self, results, jsonfile_prefix=None):
        ...
        for det in mmcv.track_iter_progress(results):
            ...
            if det is None or not isinstance(det, dict) or 'boxes_3d' not in det:
                nusc_annos[sample_token] = annos
                continue
        ...
        print(f'[Debug] _format_bbox_det: processed {processed_samples} eval samples, '
              f'{samples_with_boxes} with boxes.')
    ...
    def format_results_det(self, results, jsonfile_prefix=None):
        ...
        """
        assert isinstance(results, list), 'results must be a list'
        print(f'[Debug] format_results_det: received {len(results)} result entries.')
        print(f'[Debug] eval_sample_tokens size: {len(self.eval_sample_tokens)}')
        results_for_format = []
        tokenised_results = []
        fallback_results = []
        ...
        if tokenised_results:
            ...
            if self.eval_sample_tokens and len(effective_tokens) != len(self.eval_sample_tokens):
                missing = self.eval_sample_tokens - effective_tokens
                if missing:
                    print(f'[Warn] {len(missing)} evaluation tokens missing from detection predictions.')

            if not filtered_results and fallback_results:
                tokenised_results = []
                print('[Info] Unable to resolve detection prediction tokens; falling back to sequential alignment.')
            else:
                if not filtered_results:
                    print('[Warn] No detection predictions matched evaluation tokens; evaluation will be skipped.')
                results_for_format = filtered_results
        else:
            tokenised_results = []
        ...
        if not tokenised_results:
            ...
            if self.eval_sample_tokens and len(effective_tokens) != len(self.eval_sample_tokens):
                missing_tokens = list((self.eval_sample_tokens or set()) - resolved_tokens)[:10]
                unresolved_samples = len(self.eval_sample_tokens or []) - len(resolved_tokens)
                print(f'[Debug] format_results_det: resolved {len(resolved_tokens)} tokens, '
                      f'missing {max(unresolved_samples, 0)}. Sample missing examples: {missing_tokens}')
                print(f'[Debug] format_results_det: found {len(extra_tokens)} extra tokens (not in eval set).')

        missing_tokens_full = (self.eval_sample_tokens or set()) - resolved_tokens
        if missing_tokens_full:
            print(f'[Info] format_results_det: padding {len(missing_tokens_full)} missing tokens with empty predictions.')
            for token in missing_tokens_full:
                results_for_format.append({'token': token, 'boxes_3d': None})
        if not results_for_format:
            print('[Warn] Detection prediction results list is empty after filtering/trimming.')
        else:
            print(f'[Debug] format_results_det: using {len(results_for_format)} entries after filtering.')
        ...
        result_files = self._format_bbox_det(results_for_format, jsonfile_prefix)
        return result_files, tmp_dir
    ...
    def _evaluate_single(self,
        ...
        if 'det' in self.eval_mod:
            try:
                with open(result_path_det, 'r') as f:
                    det_data = json.load(f)
            except Exception:
                det_data = None
            if det_data is not None and 'results' in det_data:
                filtered_results = {}
                for token in det_data['results']:
                    if token in self.eval_sample_tokens:
                        filtered_results[token] = det_data['results'][token]
                det_data['results'] = filtered_results
                filtered_path_det = osp.join(
                    output_dir_det, 'filtered_results_nusc_det.json')
                with open(filtered_path_det, 'w') as f:
                    json.dump(det_data, f)
                result_path_det = filtered_path_det
