...
class UniAD(UniADTrack):
    ...
    def forward_train(self,
        ...
        # losses_track, outs_track = self.forward_track_train(img, gt_bboxes_3d, gt_labels_3d, gt_past_traj, gt_past_traj_mask, gt_inds, gt_sdc_bbox, gt_sdc_label, l2g_t, l2g_r_mat, img_metas, timestamp)
        ...
        if (not torch.distributed.is_available()
            ...
            logging.getLogger(__name__).info(
                '[UniAD] raw track losses: %s',
                {k: float(v.detach().cpu()) if torch.is_tensor(v) else v
                 for k, v in losses_track.items()})
        ...
        if self.with_occ_head:
            ...
            if (not torch.distributed.is_available()
                    ...
                    or torch.distributed.get_rank() == 0):
                logging.getLogger(__name__).info(
                    '[UniAD] raw occ losses: %s',
                    {k: float(v.detach().cpu()) if torch.is_tensor(v) else v
                     for k, v in losses_occ.items()})
        ...
        if self.with_planning_head:
            ...
            if (not torch.distributed.is_available()
                    ...
                    or torch.distributed.get_rank() == 0):
                logging.getLogger(__name__).info(
                    '[UniAD] raw planning losses: %s',
                    {k: float(v.detach().cpu()) if torch.is_tensor(v) else v
                     for k, v in losses_planning.items()})
            

            losses_planning = self.loss_weighted_and_prefixed(losses_planning, prefix='planning')
            losses.update(losses_planning)

        # # Debug logging of raw loss values (tensor or scalar)
        # if (not torch.distributed.is_available()
        #         or not torch.distributed.is_initialized()
        #         or torch.distributed.get_rank() == 0):
        ...
        #             raw_snapshot[key] = float(value.detach().cpu())
        #         else:
        #             try:
        #                 raw_snapshot[key] = float(value)
        #             except TypeError:
        #                 raw_snapshot[key] = str(value)
        #     logger.info('[UniAD] raw losses: %s', raw_snapshot)
        ...
        for k, v in losses.items():
            # tensor_v = torch.nan_to_num(v)
            # if not torch.is_tensor(tensor_v):
            #     # tensor_v = torch.tensor(float(tensor_v), device=device)
            #     tensor_v = torch.tensor(
            #         float(tensor_v), device=device, dtype=torch.float32)
            if torch.is_tensor(v):
                losses[k] = torch.nan_to_num(v).to(device)

 
            else:
                # losses[k] = torch.tensor(float(v), device=device, dtype=torch.float32)
                losses[k] = torch.tensor(
                    float(v), device=device, dtype=torch.float32,
                    requires_grad=True)

        # if not any('loss' in key for key in losses.keys()):
