...
data_root = os.environ.get('ODVLA_DATA_ROOT', '/workspace/OpenDriveVLA/volume/data/nuscenes/')
info_root = os.environ.get('ODVLA_INFO_ROOT', '/workspace/OpenDriveVLA/volume/data/infos_mini/')
...
train_llava_and_vision_tower_pipeline = [
    dict(type="LoadMultiViewImageFromFilesInCeph", to_float32=True, file_client_args=file_client_args, img_root=data_root),
    dict(type="PhotoMetricDistortionMultiViewImage"),
    dict(
        type="LoadAnnotations3D_E2E",
...
train_llava_with_track_gt_pipeline = [
    dict(type='LoadMultiViewImageFromFilesInCeph', to_float32=True,
            file_client_args=file_client_args, img_root=data_root),
    dict(type="NormalizeMultiviewImage", **img_norm_cfg),
    dict(type="PadMultiViewImage", size_divisor=32),
...
data = dict(
    ...
    test_llava_with_track_gt=dict(
        ...
        test_mode=True,
        ann_file=ann_file_test,
        pipeline=train_llava_with_track_gt_pipeline,
        patch_size=patch_size,
        canvas_size=canvas_size,
        bev_size=(bev_h_, bev_w_),
        predict_steps=predict_steps,
    ...
    train_llava_with_track_gt=dict(
        type=dataset_type,
        file_client_args=file_client_args,
        data_root=data_root,
        test_mode=True,
    ...
    train_llava_without_track_gt=dict(
        type=dataset_type,
        file_client_args=file_client_args,
        data_root=data_root,
        test_mode=True,
    ...
    train_llava_and_vision_tower=dict(
        ...
        data_root=data_root,
        ann_file=ann_file_train,
        pipeline=train_llava_and_vision_tower_pipeline,
        classes=class_names,
        modality=input_modality,
        test_mode=False,
        use_valid_flag=True,
