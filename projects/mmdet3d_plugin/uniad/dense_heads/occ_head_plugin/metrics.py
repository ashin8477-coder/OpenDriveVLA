...
def stat_scores_multiple_classes(pred, target, num_classes, average: Optional[str] = None):
    """兼容旧版 lightning.metrics.functional.classification.stat_scores_multiple_classes."""
    metric = MulticlassStatScores(num_classes=num_classes, average=None)
    scores = metric(pred, target)              # 形状: [num_classes, 4]，依次为 tp, fp, tn, fn
    tp, fp, tn, fn = scores.unbind(dim=-1)
    support = tp + fn
    if average is None:
        return tp, fp, tn, fn, support
    # 需要做聚合时复用 torchmetrics 的 reduce
    return reduce(torch.stack([tp, fp, tn, fn, support], dim=0), reduction=average)
